        ...
        <tr v-for="(clan, index) in availableclans" :key="index">
          ...
          <td>
            <button
              ...
              @click="toggleRequest(clan.clanId, clan.clanname, index)"
            >
              {{ clan.request ? 'Joined' : 'Join' }}
            </button>
          </td>
        </tr>
      </tbody>
...
import { getUserDetails } from '@/utils/fetchUserDetail';
...
export default {
  ...
  methods: {
    async fetchClans() {
      try {
        const response = await axios.get('/clanuser/api/clans');
    ...
    async fetchMembers() {
      if(this.userStore.user.clan == null) return
      try {
        const response = await axios.get(`/clanuser/api/clanusers/clan/${this.userStore.user.clan.clanId}`);
        return response.data;
      } catch (error) {
    ...
    async fetchData() {
      ...
      try {
        const [clansData, tournamentsData, membersData] = await Promise.all([
          this.fetchClans(),
        ...
        this.availableclans = clansData.map((clandata, index) => ({
        ...
        this.members = membersData.map(member => ({
          ...
          profile: "https://cdn-icons-png.flaticon.com/512/11619/11619566.png",
        ...
        this.upcomingTournaments = tournamentsData
          .filter(tournament => tournament.gameMode === 'CLANWAR')
          .map((tournament, index) => ({
            tournamentId: tournament.tournament_id,
            name: tournament.name,
            image: tournamentImage[index] || 'https://assetsio.gnwcdn.com/blackmythwukong1.jpg?width=1200&height=1200&fit=bounds&quality=70&format=jpg&auto=webp',
            description: tournament.description || 'No description available',
          }));

        this.memberssignedup = tournamentsData
          .filter(tournament => tournament.gameMode === 'CLANWAR')
          .map((person, index) => ({
            number: index + 1,
            clanName: "null",
    ...
    toggleRequest(clanId, clanname, index) {
      ...
      this.clanStore.setClan(joiningClan);
      console.log(joiningClan);
      console.log(this.userStore.user.clan);
      console.log(`222222222222222222222222222222222`);
      this.getDetails.getUserDetails();
